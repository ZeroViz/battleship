  ...
  var Battle = function (spec) {
    ...
    battle.do_deploy = function (player_id, fleet) {
      if (this.seas[player_id]) {
        var errors = this.validate_fleet(fleet, this.ruleset);
        if (errors === null) {
            this.add_fleet(player_id, fleet);
        }
        return errors;
      } else {
    ...
    battle.validate_fleet = function (fleet, ruleset) {
      var errors = [];
      var i;
      var rs = Ruleset[ruleset.name].ships;
      var required_ships = {};
      ...
      for (i = 0; i < fleet.ships.length; i += 1) {
        ...
        if (rs[v_ship.type]){
            required_ships[v_ship.type] = rs[v_ship.type]-1;
        }else{
          errors.push('ship type ' + v_ship.type + ' is invalid');  
        }

        var ship_end = this.get_ship_end(v_ship);

        //TODO replace this mess with something a little nicer
        if (ship_end === null || ship_end[0] >= ruleset.size[0] || ship_end[1] >= ruleset.size[1] ||
            ship_end[0] < 0 || ship_end[1] < 0 || v_ship.loc[0] >= ruleset.size[0] ||
            v_ship.loc[1] >= ruleset.size[1] || v_ship.loc[0] < 0 || v_ship.loc[1] < 0) {
          errors.push('ship ' + v_ship + 'exceeds the bounds of the game board');
        }
        fleet.ships[i] = v_ship;
        
        var used = [];
        var ship_used = this.get_ship_locs(v_ship);
        ship_used.forEach(function(point){
          used.forEach(function(point, point2){
      ...
      if (required_ships.carrier !== 0 ||
        ...
        errors.push('the required number of ships have not been meet ' + required_ships);
      }
    ...
    battle.do_enact = function (player_id, action) {
      if (this.seas[player_id]){
        var errors = this.validate_action(action, this.ruleset)
        if (errors === null){
        ...
        for (i2 = 0; i2 < ships.length; i2 += 1) {
          ...
          for (len = 0; len < ship.length; len += 1) {
            if (newLoc[0] === ship[len][0] && newLoc[1] === ship[len][1]) {
                affect = 'hit';
                ship_affects.push({affect: 'hit', ship: ships[i2], status_loc: len, shot_loc: newLoc})
                break;
            }
          }      
        }
        if (affect === 'miss'){
          ship_affects.push({affect: 'miss', shot_loc: newLoc})
        }
      }
      
      ship_affects.forEach(function (affect) {
        if (affect.affect === 'hit') {
          ships[affect.ship].status[affect.status_loc] = 1;
          var sunk = true;
          for (var s = 0, slen = ships[affect.ship].status.length; s < slen; s += 1) {
            if (ship.status[s] === 0){
              sunk = false;
              break;
            }
          ...
          if (sunk){
            action.reports[action.reports.length] = {affect: 'sunk', loc: affect.shot_loc, ship: {type: affect.type}}
          }else{
            action.reports[action.reports.length] = {affect: 'hit', loc: affect.shot_loc}
          }
        }else{
          action.reports[action.reports.length] = {affect: 'miss', loc: affect.shot_loc}
        }
      });
