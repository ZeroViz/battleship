    ...
    socket.on('join', function (data) {
      gameProvider.findOne({players: {id: player_id}}, function(error, game) {
        ...
        if (game) {
          ...
          // If a player disconnects and returns, we still have the mcs to use.
          // If the server restarts, we rebuild the mc when both players return.
          if (mcs[game.id]) {
            mc = mcs[game.id];
            log.debug('We already know about this game.');
          }
          else {
            if (!playersResume[game.id]) {
              playersResume[game.id] = [];
            }
            playersResume[game.id].push(player);
            if (playersResume[game.id].length > 1) {
              log.debug('We\'ve got two players %s!', JSON.stringify(playersResume[game.id]));
              var coms = {};
              playersResume[game.id].forEach(function (player) {
                coms[player.id] = player.com;
              });
              // TODO: Create a proper game object, this doesn't work.
              var mc = MatchCoordinator(game, coms);
              mcs[game.id] = mc;
              delete playersResume[game.id];
            }
            else {
              player.com('info', { message: 'Waiting for other user to resume' });
            }
          }
          if (mc) {
              socket.emit('info', { message: 'resuming game ' + game.id});
              mc.broadcast('rengage', { id: game.id,
                                       type: 'normal',
          ...
          if (players) {
            ...
            create_game(players, function (game) {
              log.debug('This is the game %s', JSON.stringify(game));
