...
var get_player_id = (function () {
  ...
  return function (session_id) {
    if (players[session_id]) {
      return players[session_id];
    } else {
      next_id += 1;
      players[session_id] = next_id;
      return next_id; 
    }
  };
}());
...
var create_game = function (players, callback) {
  idProvider.getUniqueId('saves', function (error, game_id) {
    log.debug('got unique id ' + game_id + ' with errors ' + JSON.stringify(error));
...
var on_connection = function (socket) {
  ...
  var player_id = get_player_id(hs.sessionID);
